...
canvas1.pack()
logo=ImageTk.PhotoImage(file=r'carpets1.png')

def call():
	os.system('python carpets.py')
def ExitApplication():
    root.destroy()
def call1():
...
canvas1.create_window(42, 100, window=HOME)
buttonE = tk.Button (root,text='EXIT',command=ExitApplication,bg='blue',fg='white')
canvas1.create_window(1238, 100, window=buttonE)
